...
def create_resource_by_index(i):
    ...
    cli(["namespace", "add", "--subsystem", subsystem, "--rbd-pool", pool, "--rbd-image", image,
         "--size", "16MB", "--rbd-create-image", "--load-balancing-group", "1",
         "--force", "--no-auto-visible"])
...
def check_resource_by_index(i, caplog):
    ...
    assert "No namespace" not in caplog.text
    assert "Failure listing namespaces:" not in caplog.text

# We want to fail in case we got an exception about invalid data in pb2 functions but this is
# just a warning for pytest. In order for the test to fail in such a case we need to ask pytest
# to regard this as an error
...
def test_create_get_subsys(caplog, config):
    ...
    with GatewayServer(config) as gateway:
        ...
        assert f"Setting QOS limits of namespace 1 in " \
               f"{subsystem_prefix}0: Successful" in caplog.text
        assert f"No previous QOS limits found, this is the first time the limits are set " \
               f"for namespace 1 on {subsystem_prefix}0" in caplog.text
        caplog.clear()
        cli(["namespace", "set_qos", "--subsystem", f"{subsystem_prefix}0", "--nsid", "1",
        ...
        assert f"Setting QOS limits of namespace 1 in " \
               f"{subsystem_prefix}0: Successful" in caplog.text
        assert f"No previous QOS limits found, this is the first time the limits are set " \
               f"for namespace 1 on {subsystem_prefix}0" not in caplog.text
