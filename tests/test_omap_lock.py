...
def start_servers(gatewayA, gatewayB, gw_group, addr, portA, portB, ceph_utils):
    ceph_utils.execute_ceph_monitor_command(
        "{" + f'"prefix":"nvme-gw create", "id": "{gatewayA.name}", '
        f'"pool": "{pool}", "group": "{gw_group}"' + "}"
    )
    gatewayA.serve()
    ...
    # Create new
    ceph_utils.execute_ceph_monitor_command(
        "{" + f'"prefix":"nvme-gw create", "id": "{gatewayB.name}", '
        f'"pool": "{pool}", "group": "{gw_group}"' + "}"
    )
    gatewayB.serve()
...
def test_multi_gateway_omap_reread(config, conn_omap_reread, caplog):
    ...
    # Until we create some resource on GW-B it shouldn't still have the resrouces created on GW-A,
    ...
    assert listB[num_subsystems - 1]["namespace_count"] == num_subsystems
...
def test_multi_gateway_concurrent_changes(config, image, conn_concurrent, caplog):
    ...
    while expected_warning_other_gw not in caplog.text:
        if time.time() - start_time > timeout:
            pytest.fail(f"Timeout: '{expected_warning_other_gw}' not found in caplog.text within "
                        f"{timeout} seconds.")
        time.sleep(0.1)
...
def test_multi_gateway_listener_update(config, image, conn_concurrent, caplog):
    ...
    assert listener_ret.status == 0
    assert f"Received request to create {gwB.host_name} TCP ipv4 listener for " \
