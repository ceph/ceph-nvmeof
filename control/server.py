    ...
    def _start_spdk(self, omap_state):
        ...
        if log_file_dir and log_file_dir.strip():
            ...
            if not log_file_dir.endswith("/"):
                log_file_dir += "/"
            self.spdk_log_file_path = f"{log_file_dir}spdk-{self.name}.log"

            log_files = [f"{self.spdk_log_file_path}{suffix}" for suffix in ['.bak', '.gz.bak']]
            ...
            try:
                ...
                pass
            try:
                os.rename(f"{self.spdk_log_file_path}.gz", f"{self.spdk_log_file_path}.gz.bak")
            except FileNotFoundError:
                pass
