...
class GatewayState(ABC):
    ...
    KMIP_SERVER_PREFIX = "kmip-server" + OMAP_KEY_DELIMITER
    ...
    def build_kmip_server_key(subsystem_nqn: str, address: str, port: int) -> str:
        key = GatewayState.KMIP_SERVER_PREFIX + subsystem_nqn + GatewayState.OMAP_KEY_DELIMITER
    ...
    def remove_subsystem(self, subsystem_nqn: str):
        ...
        for key in state.keys():
            ...
            if key.startswith(GatewayState.build_namespace_key(subsystem_nqn, None)):
                ...
                self._remove_key(key)
            elif key.startswith(GatewayState.build_kmip_server_key(subsystem_nqn, None, 0)):
                self._remove_key(key)
    ...
    def add_kmip_server(self, subsystem_nqn: str, address: str, port: int, val: str):
        """Adds a KMIP server to the state data store."""
        key = GatewayState.build_kmip_server_key(subsystem_nqn, address, port)
        self._add_key(key, val)

    def remove_kmip_server(self, subsystem_nqn: str, address: str, port: int):
        """Removes a KMIP server from the state data store."""
        key = GatewayState.build_kmip_server_key(subsystem_nqn, address, port)
        self._remove_key(key)
...
class GatewayStateHandler:
    ...
    def add_kmip_server(self, subsystem_nqn: str, address: str, port: int, val: str):
        """Adds a KMIP server to the state data store."""
        self.omap.add_kmip_server(subsystem_nqn, address, port, val)
        self.local.add_kmip_server(subsystem_nqn, address, port, val)

    def remove_kmip_server(self, subsystem_nqn: str, address: str, port: int):
        """Removes a KMIP server from the state data store."""
        self.omap.remove_kmip_server(subsystem_nqn, address, port)
        self.local.remove_kmip_server(subsystem_nqn, address, port)
    ...
    def update(self) -> bool:
        ...
        with self.update_is_active_lock:
            prefix_list = [
                ...
                GatewayState.KMIP_SERVER_PREFIX,
